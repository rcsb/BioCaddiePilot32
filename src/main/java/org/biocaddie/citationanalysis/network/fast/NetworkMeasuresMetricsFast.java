 ...
 *  
 */
public class NetworkMeasuresMetricsFast {
	...
	public static void main(String[] args) throws IOException {
		...
		NetworkUtilsFast.readPajekNetFile(network); 
		...
		if (flag == 2 || flag == 12)
// TODO			betweennessCentrality(network);
	 ...
	 */
	private static void pageRank(NetworkFast network, double alpha) {
...
//	    	Node node = iter.next().getValue();
	...
	//	Map<Integer, Double> size_tmp = new HashMap<Integer, Double>(); //initialize it with 1.0/numNode
	    Int2DoubleOpenHashMap size_tmp = new Int2DoubleOpenHashMap();
		double initialSize = 1.0 / network.numNode;
	    for (int key: network.nodeMap.keySet() ) 
	    	size_tmp.put(key, initialSize);
...
//	
	 ...
	 * @throws IOException
	 */
	private static void writeResultsToFile(NetworkFast network, String outFileName, double alpha) throws IOException{
